...
- O rate limit é baseado em chaves (`chat:uid`, `search:uid`, `recommendations:uid`).
...
- **TTL (Tempo de Vida)**: O cache expira automaticamente após 1 hora (60 minutos) da data de criação (`createdAt`).
...
### 3. Processamento Assíncrono de Embeddings
...
- A geração do embedding (chamada da API Gemini) é disparada em segundo plano de forma assíncrona. Ao finalizar com sucesso, o vetor e `embeddingVersion: 1` são gravados.
- Criamos a rota de conciliação administrativa `/api/admin/embeddings/reprocess` que permite reprocessar em lotes (limite de 10 itens por vez) quaisquer publicações com embeddings pendentes (`embeddingVersion == 0`).
...
- **Mascaramento de Dados Sensíveis**: Filtra chaves específicas de metadados como `email`, `name`, `birthDate`, `sensitivities`, `interests` e `notes`, substituindo o valor por `[REDACTED]` antes de emitir o log, garantindo conformidade com a LGPD e privacidade.
